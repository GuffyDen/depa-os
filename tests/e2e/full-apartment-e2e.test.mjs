...
test("FULL APARTMENT E2E — linked commercial, production, portal and finance lifecycle", async () => {
  ...
  const rcAddresses = [id("rc_address_1"), id("rc_address_2"), id("rc_address_3")], selectedAddress = rcAddresses[1];
  ...
  await q("INSERT INTO residential_complexes(id,name,normalized_name,city,status,created_by_user_id,created_at,updated_at) VALUES($1,'Тестовый ЖК E2E','тестовый жк e2e','Владивосток','ACTIVE',$2,$3,$3)", [rc, user, now]);
  for (const [position, addressId] of rcAddresses.entries()) await q("INSERT INTO residential_complex_addresses(id,residential_complex_id,address,normalized_address,position,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$6)", [addressId, rc, `Тестовая улица, ${position + 1}`, `тестовая улица, ${position + 1}`, position, now]);
  ...
  await q("INSERT INTO inspections(id,order_id,residential_complex,residential_complex_id,residential_complex_address_id,address,apartment_number,area_sqm,scheduled_at,scheduled_start_at,scheduled_end_at,inspector_user_id,result_comment,created_at,updated_at) VALUES($1,$2,'Тестовый ЖК E2E',$3,$4,'Тестовая улица, 2','101',65,$5,$5,$6,$7,'Приёмка завершена',$8,$8)", [inspection, inspectionOrder, rc, selectedAddress, now + day, now + day + 3600, user, now]);
  ...
  await q("INSERT INTO design_projects(id,order_id,residential_complex,residential_complex_id,residential_complex_address_id,address,apartment_number,area_sqm,status,actual_end_date,created_at,updated_at) VALUES($1,$2,'Тестовый ЖК E2E',$3,$4,'Тестовая улица, 2','101',65,'COMPLETED',$5,$5,$5)", [design, designOrder, rc, selectedAddress, now]);
  ...
  await q("INSERT INTO estimates(id,client_id,responsible_user_id,residential_complex_id,residential_complex_address_id,address,apartment_number,area_sqm,source_lead_id,source_order_id,status,created_by_user_id,created_at,updated_at) VALUES($1,$2,$3,$4,$5,'Тестовая улица, 2','101',65,$6,$7,'ACTIVE',$3,$8,$8)", [estimate, client, user, rc, selectedAddress, lead, designOrder, now]);
  ...
  await q("INSERT INTO renovation_order_details(id,order_id,residential_complex,residential_complex_id,residential_complex_address_id,address,apartment_number,area_sqm,approved_estimate_version_id,created_at,updated_at) VALUES($1,$2,'Тестовый ЖК E2E',$3,$4,'Тестовая улица, 2','101',65,$5,$6,$6)", [id("renovation_details"), renovationOrder, rc, selectedAddress, estimateV2, now]);
  ...
  await q("INSERT INTO contract_versions(id,contract_id,version,status,contract_date,estimate_version_id,contract_amount_kopecks,estimated_materials_budget_kopecks,client_snapshot_json,company_snapshot_json,property_snapshot_json,terms_snapshot_json,document_snapshot_json,signed_at,signed_by_user_id,created_by_user_id,created_at,updated_at) VALUES($1,$2,1,'SIGNED',$3,$4,110000000,32000000,$5,$6,$7,'{}','{}',$3,$8,$8,$3,$3)", [contractV1, contract, now, estimateV2, JSON.stringify({ id: client, name: "Тестовый Клиент E2E" }), JSON.stringify({ name: "DEPA TEST" }), JSON.stringify({ residentialComplexId: rc, residentialComplexAddressId: selectedAddress, address: "Тестовая улица, 2", apartment: "101", areaSqm: 65 }), user]);
  await q("UPDATE contracts SET current_version_id=$1,signed_version_id=$1 WHERE id=$2", [contractV1, contract]);
  await q("INSERT INTO projects(id,order_id,client_id,name,residential_complex,residential_complex_id,residential_complex_address_id,address,apartment,area_sqm,status,contract_amount_kopecks,responsible_user_id,estimated_materials_budget_kopecks,approved_estimate_version_id,contract_id,created_by_user_id,created_at,updated_at) VALUES($1,$2,$3,'Тестовый ЖК E2E · кв. 101','Тестовый ЖК E2E',$4,$5,'Тестовая улица, 2','101',65,'ACTIVE',110000000,$6,32000000,$7,$8,$6,$9,$9)", [project, renovationOrder, client, rc, selectedAddress, user, estimateV2, contract, now]);
  const exactAddressChain = await q("SELECT i.residential_complex_address_id inspection,dp.residential_complex_address_id design,e.residential_complex_address_id estimate,rod.residential_complex_address_id renovation,p.residential_complex_address_id project,i.address inspection_text,p.address project_text FROM inspections i JOIN design_projects dp ON dp.id=$1 JOIN estimates e ON e.id=$2 JOIN renovation_order_details rod ON rod.order_id=$3 JOIN projects p ON p.id=$4 WHERE i.id=$5", [design, estimate, renovationOrder, project, inspection]);
  assert.deepEqual(exactAddressChain.rows[0], { inspection: selectedAddress, design: selectedAddress, estimate: selectedAddress, renovation: selectedAddress, project: selectedAddress, inspection_text: "Тестовая улица, 2", project_text: "Тестовая улица, 2" });
  ...
  // 259–294: Apartment Passport v1 — large real dataset, immutable v1, republish v2 and safe client history.
  await q(`INSERT INTO attachments(id,project_id,storage_key,original_filename,mime_type,size_bytes,storage_provider,blob_url,uploaded_by_user_id,entity_type,entity_id,category,visibility,upload_status,metadata_json,completed_at,linked_at,client_visible,created_at,updated_at)
    SELECT 'e2e_passport_daily_'||g,$1,'test-storage/passport-daily-'||g||'.jpg','daily-'||g||'.jpg','image/jpeg',128,'VERCEL_BLOB','https://test.private.blob.vercel-storage.com/passport-daily-'||g||'.jpg',$2,'DailyReport',$3,'DAILY_REPORT','CLIENT','LINKED','{}',$4,$4,1,$4,$4 FROM generate_series(1,100) g`,[project,user,report,now]);
  await q(`INSERT INTO attachments(id,project_id,photo_requirement_id,storage_key,original_filename,mime_type,size_bytes,storage_provider,blob_url,uploaded_by_user_id,entity_type,entity_id,category,visibility,upload_status,metadata_json,completed_at,linked_at,client_visible,created_at,updated_at)
    SELECT 'e2e_passport_hidden_'||g,$1,$2,'test-storage/passport-hidden-'||g||'.jpg','hidden-'||g||'.jpg','image/jpeg',128,'VERCEL_BLOB','https://test.private.blob.vercel-storage.com/passport-hidden-'||g||'.jpg',$3,'Task',$4,'HIDDEN_WORK','CLIENT','LINKED','{}',$5,$5,1,$5,$5 FROM generate_series(1,20) g`,[project,requirement,user,id("task_sockets"),now]);
  await q(`INSERT INTO attachments(id,project_id,storage_key,original_filename,mime_type,size_bytes,storage_provider,blob_url,uploaded_by_user_id,entity_type,entity_id,category,visibility,upload_status,metadata_json,completed_at,linked_at,client_visible,created_at,updated_at)
    SELECT 'e2e_passport_doc_'||g,$1,'test-storage/passport-doc-'||g||'.pdf','document-'||g||'.pdf','application/pdf',256,'VERCEL_BLOB','https://test.private.blob.vercel-storage.com/passport-doc-'||g||'.pdf',$2,'Project',$1,'OTHER','CLIENT','LINKED','{}',$3,$3,1,$3,$3 FROM generate_series(1,10) g`,[project,user,now]);
  for(let n=2;n<=5;n+=1){const work=id(`passport_work_${n}`),version=id(`passport_work_version_${n}`);await q("INSERT INTO additional_works(id,project_id,client_id,order_id,contract_id,number,title,status,responsible_user_id,created_by_user_id,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,'DRAFT',$8,$8,$9,$9)",[work,project,client,renovationOrder,contract,`ДР-E2E-00${n}`,`Допработа паспорта ${n}`,user,now]);await q("INSERT INTO additional_work_versions(id,additional_work_id,project_id,version,title,amount_kopecks,schedule_delta_days,status,reason,client_description,schedule_impact_type,task_creation_mode,approved_at,created_by_user_id,created_at,updated_at) VALUES($1,$2,$3,1,$4,$5,0,'APPROVED','CLIENT_REQUEST',$4,'NO_IMPACT','AFTER_APPROVAL',$6,$7,$6,$6)",[version,work,project,`Допработа паспорта ${n}`,n*100000,now,user]);await q("UPDATE additional_works SET status='APPROVED',current_version_id=$1,approved_version_id=$1 WHERE id=$2",[version,work])}
  const financeBeforePassport=await q("SELECT (SELECT COUNT(*) FROM financial_transactions WHERE project_id=$1)::int transactions,(SELECT COUNT(*) FROM obligations WHERE project_id=$1)::int obligations,(SELECT balance_kopecks FROM cashboxes WHERE id=$2)::int balance",[project,cashbox]);
  const passportId=id("apartment_passport");
  await Promise.all([1,2,3].map(n=>q("INSERT INTO apartment_passports(id,project_id,client_id,status,created_at,updated_at) VALUES($1,$2,$3,'READY',$4,$4) ON CONFLICT(project_id) DO NOTHING",[`${passportId}_${n}`,project,client,now])));
  const actualPassport=(await q("SELECT id FROM apartment_passports WHERE project_id=$1",[project])).rows[0].id;assert.ok(actualPassport);assert.equal((await q("SELECT COUNT(*)::int count FROM apartment_passports WHERE project_id=$1",[project])).rows[0].count,1);
  for(const [position,key] of ["GENERAL","DESIGN","ESTIMATE","ADDITIONAL_WORKS","PRODUCTION","DAILY_PHOTOS","HIDDEN_WORKS","FINAL_PHOTOS","HANDOVER","DEFECT_HISTORY","DOCUMENTS","WARRANTY"].entries())await q("INSERT INTO apartment_passport_sections(id,passport_id,section_key,enabled,position,created_at,updated_at) VALUES($1,$2,$3,1,$4,$5,$5)",[id(`passport_section_${position}`),actualPassport,key,position,now]);
  async function sqlPublish(versionNumber,releaseNote,customNote){const versionId=id(`passport_v${versionNumber}`),snapshot={schemaVersion:1,general:{clientName:"Тестовый Клиент E2E",apartment:"101",areaSqm:65},additionalWorks:Array.from({length:5},(_,index)=>({id:`work-${index+1}`})),commercial:{currentCommercialTotalKopecks:116000000},handover:{status:"ACCEPTED",actualHandoverAt:now},customNote};await q("INSERT INTO apartment_passport_versions(id,passport_id,version_number,snapshot_json,source_manifest_json,source_hash,release_note,published_by_user_id,published_at,created_at) VALUES($1,$2,$3,$4::jsonb,$5::jsonb,$6,$7,$8,$9,$9)",[versionId,actualPassport,versionNumber,JSON.stringify(snapshot),JSON.stringify({projectId:project}),String(versionNumber).repeat(64).slice(0,64),releaseNote,user,now+versionNumber]);await q(`INSERT INTO apartment_passport_version_attachments(id,passport_version_id,attachment_id,section_key,position,caption,display_name_snapshot,mime_type_snapshot,size_bytes_snapshot,created_at)
    SELECT $1||':'||ROW_NUMBER() OVER(ORDER BY a.created_at,a.id),$2,a.id,CASE WHEN a.category='DAILY_REPORT' THEN 'DAILY_PHOTOS' WHEN a.category='HIDDEN_WORK' THEN 'HIDDEN_WORKS' WHEN a.category IN('HANDOVER_DEFECT','HANDOVER_DEFECT_RESOLUTION') THEN 'DEFECT_HISTORY' WHEN a.category='ADDITIONAL_WORK' THEN 'ADDITIONAL_WORKS' ELSE 'DOCUMENTS' END,ROW_NUMBER() OVER(ORDER BY a.created_at,a.id)-1,NULL,a.original_filename,a.mime_type,a.size_bytes,$3 FROM attachments a WHERE a.project_id=$4 AND a.upload_status='LINKED' AND a.deleted_at IS NULL AND (a.client_visible=1 OR a.visibility='CLIENT')`,[`e2e_passport_ref_v${versionNumber}`,versionId,now,project]);await q("UPDATE apartment_passports SET status='PUBLISHED',current_published_version_id=$1,published_at=$2,published_by_user_id=$3,custom_note=$4,updated_at=$2 WHERE id=$5",[versionId,now+versionNumber,user,customNote,actualPassport]);return versionId}
  const v1Id=await sqlPublish(1,"Первая полная версия",null),v1Snapshot=(await q("SELECT snapshot_json FROM apartment_passport_versions WHERE id=$1",[v1Id])).rows[0].snapshot_json;
  const v1Facts=(await q("SELECT (SELECT COUNT(*) FROM apartment_passport_versions WHERE passport_id=$1)::int versions,(SELECT COUNT(*) FROM apartment_passport_version_attachments va JOIN apartment_passport_versions v ON v.id=va.passport_version_id WHERE v.passport_id=$1 AND va.section_key IN('DAILY_PHOTOS','HIDDEN_WORKS','DEFECT_HISTORY'))::int photos",[actualPassport])).rows[0];assert.equal(v1Facts.versions,1);assert.ok(v1Facts.photos>=123);assert.equal(v1Snapshot.additionalWorks.length,5);
  await assert.rejects(q("UPDATE apartment_passport_versions SET release_note='tampered' WHERE id=$1",[v1Id]),/immutable/i);
  await assert.rejects(q("DELETE FROM attachments WHERE id='e2e_passport_daily_1'"),/retained|foreign key/i);
  await q("UPDATE apartment_passports SET custom_note='Добавлены гарантийные материалы',financial_summary_enabled=1 WHERE id=$1",[actualPassport]);
  const v2Id=await sqlPublish(2,"Добавлены гарантийные материалы","Добавлены гарантийные материалы");
  const versions=await q("SELECT id,version_number FROM apartment_passport_versions WHERE passport_id=$1 ORDER BY version_number DESC",[actualPassport]);assert.equal(versions.rowCount,2);assert.equal(versions.rows[0].version_number,2);
  const unchanged=(await q("SELECT snapshot_json FROM apartment_passport_versions WHERE id=$1",[v1Id])).rows[0].snapshot_json;assert.deepEqual(unchanged,v1Snapshot);
  const clientV2=(await q("SELECT v.version_number,v.snapshot_json,(SELECT COUNT(*) FROM apartment_passport_versions h WHERE h.passport_id=ap.id)::int history FROM apartment_passports ap JOIN projects p ON p.id=ap.project_id JOIN apartment_passport_versions v ON v.id=ap.current_published_version_id WHERE ap.project_id=$1 AND ap.client_id=$2 AND p.client_id=$2 AND ap.status='PUBLISHED'",[project,client])).rows[0];assert.equal(clientV2.version_number,2);assert.equal(clientV2.history,2);assert.equal(clientV2.snapshot_json.customNote,"Добавлены гарантийные материалы");
  const clientV1=(await q("SELECT version_number,snapshot_json FROM apartment_passport_versions WHERE id=$1 AND passport_id=$2",[v1Id,actualPassport])).rows[0];assert.equal(clientV1.version_number,1);assert.equal(clientV1.snapshot_json.customNote,null);
  const page1=(await q("SELECT attachment_id,position FROM apartment_passport_version_attachments WHERE passport_version_id=$1 AND section_key='DAILY_PHOTOS' ORDER BY position,id LIMIT 50",[v2Id])).rows;assert.equal(page1.length,50);
  const page2=(await q("SELECT attachment_id,position FROM apartment_passport_version_attachments WHERE passport_version_id=$1 AND section_key='DAILY_PHOTOS' AND position>$2 ORDER BY position,id LIMIT 50",[v2Id,page1.at(-1).position])).rows;assert.equal(page2.length,50);assert.equal(new Set([...page1,...page2].map(item=>item.attachment_id)).size,100);
  assert.equal((await q("SELECT COUNT(*)::int count FROM apartment_passports ap JOIN projects p ON p.id=ap.project_id WHERE ap.project_id=$1 AND ap.client_id=$2 AND p.client_id=$2",[project,otherClient])).rows[0].count,0);
  const financeAfterPassport=await q("SELECT (SELECT COUNT(*) FROM financial_transactions WHERE project_id=$1)::int transactions,(SELECT COUNT(*) FROM obligations WHERE project_id=$1)::int obligations,(SELECT balance_kopecks FROM cashboxes WHERE id=$2)::int balance",[project,cashbox]);assert.deepEqual(financeAfterPassport.rows[0],financeBeforePassport.rows[0]);

  // 295–308 + full relation and independent money reconciliation.
  const relationAudit = await q(`SELECT
    ...
    (SELECT COUNT(*) FROM project_handover_defects WHERE project_id=$4 AND status='ACCEPTED')::int accepted_defects,
    (SELECT COUNT(*) FROM apartment_passports WHERE project_id=$4 AND status='PUBLISHED')::int passports,
    (SELECT COUNT(*) FROM apartment_passport_versions v JOIN apartment_passports ap ON ap.id=v.passport_id WHERE ap.project_id=$4)::int passport_versions`, [lead, client, estimateV2, project, renovationOrder, contract, plan]);
  assert.deepEqual(relationAudit.rows[0], { lead_client: 1, client_orders: 3, approved_estimate: 1, client_contract: 1, project_links: 1, plans: 1, stages: 3, tasks: 10, dependencies: 3, report_tasks: 1, files: 135, payments: 5, handovers: 1, accepted_defects: 1, passports:1, passport_versions:2 });
