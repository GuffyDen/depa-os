...
export async function getClient(actor: AuthUser, clientId: string) {
  ...
  const assignedProjectWhere = actor.role === "OWNER" || access.scopes.projects === "ALL" ? "" : ` AND (EXISTS (SELECT 1 FROM user_project_access a WHERE a.project_id=p.id AND a.user_id=$2) OR p.manager_employee_id=$3 OR p.foreman_employee_id=$3)`;
  const assignedTaskWhere = actor.role === "OWNER" || access.scopes.tasks === "ALL" ? "" : " AND (t.assignee_employee_id=$2 OR t.created_by_user_id=$3)";
  const assignedDocumentWhere = actor.role === "OWNER" || access.scopes.documents === "ALL" ? "" : ` AND (a.entity_type='Client' OR EXISTS (SELECT 1 FROM projects p LEFT JOIN user_project_access upa ON upa.project_id=p.id AND upa.user_id=$2 WHERE p.id=a.project_id AND (upa.id IS NOT NULL OR p.manager_employee_id=$3 OR p.foreman_employee_id=$3)))`;
