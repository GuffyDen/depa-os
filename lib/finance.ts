...
export async function getFinanceOverview(actor: AuthUser) {
  ...
  const [boxes, transactions, projects, clients, projectEconomics] = await Promise.all([
    ...
    actor.role === "OWNER"
      ? query<ProjectEconomicsRow>(`SELECT project_id,
      ...
      FROM financial_transactions WHERE project_id IS NOT NULL GROUP BY project_id`)
      : query<ProjectEconomicsRow>(`SELECT ft.project_id,
        COALESCE(SUM(CASE WHEN ft.type = 'INCOME' THEN ft.amount_kopecks ELSE 0 END), 0) AS income_kopecks,
        COALESCE(SUM(CASE WHEN ft.type = 'EXPENSE' THEN ft.amount_kopecks ELSE 0 END), 0) AS expense_kopecks,
        COALESCE(SUM(CASE WHEN ft.type = 'REFUND' THEN ft.amount_kopecks ELSE 0 END), 0) AS refund_kopecks
      FROM financial_transactions ft JOIN user_project_access a ON a.project_id = ft.project_id
      WHERE a.user_id = $1 AND ft.project_id IS NOT NULL GROUP BY ft.project_id`, [actor.id]),
  ]);
  ...
  return {
    ...
    projects: projects.map((project) => {
      ...
      return { id: project.id, name: project.name, clientId: project.client_id, incomeKopecks, expenseKopecks, refundKopecks, ...projectLedgerTotals(incomeKopecks, expenseKopecks, refundKopecks) };
    }),
    clients,
...
export async function createFinanceOperation(actor: AuthUser, input: CreateFinanceOperationInput) {
  ...
  const title = cleanText(input.title, 180) || (type === "TRANSFER" ? "Перемещение между кассами" : category || type);
  ...
  if (type === "TRANSFER") {
    ...
    const expenseError = validateExpense(expenseType, category, projectId);
    if (expenseError) throw new FinanceError(expenseError);
    if (expenseType === "ADMIN") { projectId = null; clientId = null; showToClient = false; }
  } else if (type === "INCOME") {
