...
export async function getClientPortalHome(user: ClientPortalUser, requestedProjectId?: string) {
  ...
  const [additionalWorks, commercial, handover] = await Promise.all([listClientAdditionalWorks(user, project.id), additionalWorkCommercialSummary(project.id), first<Record<string, unknown>>("SELECT id,status,accepted_at FROM project_handovers WHERE project_id=$1 AND status IN('AWAITING_CLIENT_INSPECTION','CORRECTIONS_REQUIRED','REINSPECTION_REQUIRED','ACCEPTED')", [project.id])]);
