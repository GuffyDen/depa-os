...
export async function getClientPortalHome(user: ClientPortalUser, requestedProjectId?: string) {
  ...
  if (selected.kind === "DESIGN") {
    ...
    return { user: { name: user.clientName }, objects, current: { ...selected, mode: "DESIGN_ONLY", progress: design.progress, progressLabel: "Готовность дизайн-проекта", publishedForecast: null, stages: [], delays: [], reports: [], hiddenWorks: [], payments: { dueKopecks: 0, confirmedKopecks: 0, obligations: [], claims: [], history: [], unappliedKopecks: 0 }, documents: [] } };
  }
  ...
  const obligations = await query(`SELECT o.id,o.obligation_type,o.stage_id,s.name stage_name,o.amount_kopecks,o.paid_kopecks,o.amount_kopecks-o.paid_kopecks remaining_kopecks,o.status,o.due_date FROM obligations o LEFT JOIN project_stages s ON s.id=o.stage_id WHERE o.project_id=$1 AND o.counterparty_type='CLIENT' AND o.counterparty_id=$2 AND o.status<>'CANCELLED' ORDER BY o.due_date NULLS LAST,o.created_at`, [project.id, user.clientId]);
