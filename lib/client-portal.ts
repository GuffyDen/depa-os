...
export type ClientPortalAction = { type: "FINAL_HANDOVER" | "ADDITIONAL_WORK_APPROVAL" | "STAGE_ACCEPTANCE" | "DEFECT_REINSPECTION" | "PAYMENT_DUE" | "PAYMENT_CLAIM_STATUS"; priority: number; eyebrow: string; title: string; cta: string; destination: "HANDOVER" | "ADDITIONAL_WORKS" | "WORK" | "PAYMENTS"; entityId?: string; amountKopecks?: number };

export function selectClientPortalAction(input: { handover?: { id?: unknown; status?: unknown } | null; additionalWorks?: Record<string, unknown>[]; stages?: Record<string, unknown>[]; dueKopecks?: number; claims?: Record<string, unknown>[] }): ClientPortalAction | null {
  const handoverStatus = String(input.handover?.status ?? "");
  if (input.handover && ["AWAITING_CLIENT_INSPECTION", "CORRECTIONS_REQUIRED"].includes(handoverStatus)) return { type: "FINAL_HANDOVER", priority: 1, eyebrow: "ФИНАЛЬНАЯ СДАЧА", title: handoverStatus === "CORRECTIONS_REQUIRED" ? "DEPA устраняет замечания" : "Квартира готова к осмотру", cta: "Открыть сдачу", destination: "HANDOVER", entityId: String(input.handover.id ?? "") };
  const proposal = input.additionalWorks?.find((item) => item.status === "AWAITING_CLIENT_APPROVAL");
  if (proposal) return { type: "ADDITIONAL_WORK_APPROVAL", priority: 2, eyebrow: "ТРЕБУЕТСЯ СОГЛАСОВАНИЕ", title: String(proposal.title ?? proposal.number ?? "Дополнительные работы"), cta: "Открыть предложение", destination: "ADDITIONAL_WORKS", entityId: String(proposal.id ?? ""), amountKopecks: Number(proposal.amount_kopecks ?? 0) };
  const stage = input.stages?.find((item) => item.acceptance_status === "AWAITING_ACCEPTANCE" || item.acceptanceStatus === "AWAITING_ACCEPTANCE");
  if (stage) return { type: "STAGE_ACCEPTANCE", priority: 3, eyebrow: "ТРЕБУЕТСЯ ПРИЁМКА ЭТАПА", title: String(stage.name ?? "Завершённый этап"), cta: "Открыть этап", destination: "WORK", entityId: String(stage.id ?? "") };
  if (input.handover && handoverStatus === "REINSPECTION_REQUIRED") return { type: "DEFECT_REINSPECTION", priority: 4, eyebrow: "ПОВТОРНЫЙ ОСМОТР", title: "Исправления готовы к проверке", cta: "Проверить замечания", destination: "HANDOVER", entityId: String(input.handover.id ?? "") };
  if (Number(input.dueKopecks ?? 0) > 0) return { type: "PAYMENT_DUE", priority: 5, eyebrow: "К ОПЛАТЕ", title: "Есть сумма к оплате", cta: "Открыть оплаты", destination: "PAYMENTS", amountKopecks: Number(input.dueKopecks) };
  const claim = input.claims?.find((item) => item.status === "PENDING");
  if (claim) return { type: "PAYMENT_CLAIM_STATUS", priority: 6, eyebrow: "ОПЛАТА ПРОВЕРЯЕТСЯ", title: "Мы проверяем поступление", cta: "Посмотреть статус", destination: "PAYMENTS", entityId: String(claim.id ?? ""), amountKopecks: Number(claim.claimed_amount_kopecks ?? 0) };
  return null;
}
...
export async function getClientPortalHome(user: ClientPortalUser, requestedProjectId?: string) {
  type ProjectRow = { id: string; name: string; residential_complex_address_id: string | null; residential_complex: string | null; residential_complex_name: string | null; address: string; apartment: string; published_forecast_end_date: number | null; contract_amount_kopecks: number; payment_plan_version: number };
  type DesignRow = { id: string; order_id: string; title: string; residential_complex_address_id: string | null; residential_complex: string | null; residential_complex_name: string | null; address: string; apartment_number: string; status: string; progress: number };
  type InspectionRow = { id: string; order_id: string; title: string; residential_complex_address_id: string | null; residential_complex: string | null; residential_complex_name: string | null; address: string; apartment_number: string; status: string; scheduled_at: number; completed_at: number | null };
  const [projects, designs, inspections] = await Promise.all([
    query<ProjectRow>(`SELECT p.id,p.name,p.residential_complex_address_id,p.residential_complex,rc.name residential_complex_name,COALESCE(rca.address,p.address) address,p.apartment,p.published_forecast_end_date,p.contract_amount_kopecks,p.payment_plan_version FROM projects p LEFT JOIN residential_complexes rc ON rc.id=p.residential_complex_id LEFT JOIN residential_complex_addresses rca ON rca.id=p.residential_complex_address_id WHERE p.client_id=$1 AND p.status<>'ARCHIVED' ORDER BY p.created_at DESC`, [user.clientId]),
    query<DesignRow>(`SELECT dp.id,dp.order_id,o.title,dp.residential_complex_address_id,dp.residential_complex,rc.name residential_complex_name,COALESCE(rca.address,dp.address) address,dp.apartment_number,dp.status,COALESCE((SELECT ROUND(100.0*COUNT(*) FILTER(WHERE ds.status='COMPLETED')/NULLIF(COUNT(*),0))::int FROM design_project_stages ds WHERE ds.design_project_id=dp.id AND ds.archived_at IS NULL),CASE WHEN dp.status='COMPLETED' THEN 100 ELSE 0 END) progress FROM design_projects dp JOIN orders o ON o.id=dp.order_id LEFT JOIN residential_complexes rc ON rc.id=dp.residential_complex_id LEFT JOIN residential_complex_addresses rca ON rca.id=dp.residential_complex_address_id WHERE o.client_id=$1 AND o.status<>'CANCELLED' ORDER BY o.created_at DESC`, [user.clientId]),
    query<InspectionRow>(`SELECT i.id,i.order_id,o.title,i.residential_complex_address_id,i.residential_complex,rc.name residential_complex_name,COALESCE(rca.address,i.address) address,i.apartment_number,o.status,i.scheduled_at,o.completed_at FROM inspections i JOIN orders o ON o.id=i.order_id LEFT JOIN residential_complexes rc ON rc.id=i.residential_complex_id LEFT JOIN residential_complex_addresses rca ON rca.id=i.residential_complex_address_id WHERE o.client_id=$1 AND o.status<>'CANCELLED' ORDER BY o.created_at DESC`, [user.clientId]),
  ]);
  const sameApartment = (left: { residential_complex_address_id: string | null; address: string; apartment?: string; apartment_number?: string }, right: { residential_complex_address_id: string | null; address: string; apartment?: string; apartment_number?: string }) => {
    const sameAddress = left.residential_complex_address_id && right.residential_complex_address_id ? left.residential_complex_address_id === right.residential_complex_address_id : left.address.trim().toLocaleLowerCase("ru-RU") === right.address.trim().toLocaleLowerCase("ru-RU");
    return sameAddress && String(left.apartment ?? left.apartment_number ?? "").trim().toLocaleLowerCase("ru-RU") === String(right.apartment ?? right.apartment_number ?? "").trim().toLocaleLowerCase("ru-RU");
  };
  const objects = [
    ...
    ...designs.filter((design) => !projects.some((project) => sameApartment(project, design))).map((design) => ({ id: design.id, kind: "DESIGN" as const, title: design.residential_complex_name ?? design.residential_complex ?? design.title, subtitle: `${design.address} · кв.${design.apartment_number}` })),
    ...inspections.filter((inspection) => !projects.some((project) => sameApartment(project, inspection)) && !designs.some((design) => sameApartment(design, inspection))).map((inspection) => ({ id: inspection.id, kind: "INSPECTION" as const, title: inspection.residential_complex_name ?? inspection.residential_complex ?? inspection.title, subtitle: `${inspection.address} · кв.${inspection.apartment_number}` })),
  ];
  ...
  if (selected.kind === "DESIGN") {
    ...
    const designStages = await query<Record<string, unknown>>("SELECT id,name,status,position FROM design_project_stages WHERE design_project_id=$1 AND archived_at IS NULL ORDER BY position", [design.id]);
    return { user: { name: user.clientName }, objects, current: { ...selected, mode: "DESIGN_ONLY", serviceLabel: "Дизайн-проект", serviceStatus: design.status, progress: design.progress, progressLabel: "Готовность дизайн-проекта", publishedForecast: null, currentPhase: designStages.find((stage) => stage.status === "IN_PROGRESS" || stage.status === "WAITING_CLIENT")?.name ?? (design.status === "COMPLETED" ? "Дизайн завершён" : "Планирование"), action: null, latestUpdate: null, latestPhotos: [], stages: designStages.map((stage) => ({ id: stage.id, name: stage.name, status: stage.status, progressPercent: stage.status === "COMPLETED" ? 100 : 0, acceptanceStatus: "NOT_REQUIRED", tasks: [] })), delays: [], reports: [], hiddenWorks: [], additionalWorks: [], handover: null, commercial: null, payments: { dueKopecks: 0, confirmedKopecks: 0, obligations: [], claims: [], history: [], unappliedKopecks: 0 }, documents: [] } };
  }
  if (selected.kind === "INSPECTION") {
    const inspection = inspections.find((item) => item.id === selected.id)!;
    const completed = inspection.status === "COMPLETED";
    return { user: { name: user.clientName }, objects, current: { ...selected, mode: "INSPECTION_ONLY", serviceLabel: "Приёмка квартиры", serviceStatus: inspection.status, progress: completed ? 100 : inspection.status === "IN_PROGRESS" ? 50 : 0, progressLabel: completed ? "Приёмка завершена" : "Статус приёмки", publishedForecast: null, currentPhase: completed ? "Результат готов" : inspection.status === "IN_PROGRESS" ? "Специалист проводит осмотр" : "Приёмка запланирована", scheduledAt: inspection.scheduled_at, action: null, latestUpdate: completed ? { type: "INSPECTION", title: "Приёмка завершена", occurredAt: inspection.completed_at } : { type: "INSPECTION", title: "Приёмка запланирована", occurredAt: inspection.scheduled_at }, latestPhotos: [], stages: [], delays: [], reports: [], hiddenWorks: [], additionalWorks: [], handover: null, commercial: null, payments: { dueKopecks: 0, confirmedKopecks: 0, obligations: [], claims: [], history: [], unappliedKopecks: 0 }, documents: [] } };
  }
  ...
  const stages = plan ? await query<Record<string, unknown>>(`SELECT s.id,s.name,s.status,s.sort_order,s.weight_within_project,s.acceptance_status,s.client_acceptance_required,COALESCE((SELECT SUM(CASE WHEN t.status='CANCELLED' THEN 0 ELSE COALESCE(t.weight_within_stage,0)*CASE WHEN t.progress_type='BINARY' THEN CASE WHEN t.status='COMPLETED' THEN 100 ELSE 0 END ELSE LEAST(100,100*COALESCE(t.completed_quantity,0)/NULLIF(t.planned_quantity,0)) END END)/NULLIF(SUM(CASE WHEN t.status='CANCELLED' THEN 0 ELSE COALESCE(t.weight_within_stage,0) END),0) FROM tasks t WHERE t.stage_id=s.id AND t.archived_at IS NULL),0) progress_percent FROM project_stages s WHERE s.production_plan_id=$1 AND s.archived_at IS NULL ORDER BY s.sort_order`, [plan.id]) : [];
  ...
  const matchingDesign = designs.find((design) => sameApartment(project, design));
  ...
  const [delays, reports, hiddenWorks, obligations, claims, history, unapplied, documents, additionalWorks, commercial, handover, passport] = await Promise.all([
    query("SELECT id,category,'Срок скорректирован'::text reason,client_comment,start_date,end_date,days FROM project_delays WHERE project_id=$1 AND client_visible=1 ORDER BY start_date DESC", [project.id]),
    query(`SELECT dr.id,dr.report_date,CASE WHEN dr.comment_client_visible=1 THEN dr.comment ELSE NULL END comment,COUNT(a.id)::int photo_count,COALESCE(json_agg(json_build_object('id',a.id,'filename',a.original_filename,'url','/api/client/files/'||a.id) ORDER BY a.created_at DESC) FILTER(WHERE a.id IS NOT NULL),'[]') photos FROM daily_reports dr LEFT JOIN LATERAL (SELECT a.id,a.original_filename,a.created_at FROM attachments a WHERE a.entity_type='DailyReport' AND a.entity_id=dr.id AND a.upload_status='LINKED' AND a.deleted_at IS NULL AND (a.visibility='CLIENT' OR a.client_visible=1) ORDER BY a.created_at DESC LIMIT 8) a ON TRUE WHERE dr.project_id=$1 GROUP BY dr.id ORDER BY dr.report_date DESC LIMIT 6`, [project.id]),
    query(`SELECT r.id,r.name,r.description,t.title task_name,COUNT(a.id)::int photo_count,COALESCE(json_agg(json_build_object('id',a.id,'filename',a.original_filename,'url','/api/client/files/'||a.id) ORDER BY a.created_at) FILTER(WHERE a.id IS NOT NULL),'[]') photos FROM task_photo_requirements r JOIN tasks t ON t.id=r.task_id LEFT JOIN attachments a ON a.photo_requirement_id=r.id AND a.upload_status='LINKED' AND a.deleted_at IS NULL AND (a.visibility='CLIENT' OR a.client_visible=1) WHERE t.project_id=$1 AND t.client_visible=1 GROUP BY r.id,t.title,t.position,r.position ORDER BY t.position,r.position`, [project.id]),
    query<Record<string, unknown>>(`SELECT o.id,o.obligation_type,o.stage_id,COALESCE(s.name,aw.title,'Оплата по объекту') stage_name,o.amount_kopecks,o.paid_kopecks,o.amount_kopecks-o.paid_kopecks remaining_kopecks,o.status,o.due_date FROM obligations o LEFT JOIN project_stages s ON s.id=o.stage_id LEFT JOIN additional_works aw ON aw.id=o.additional_work_id WHERE o.project_id=$1 AND o.counterparty_type='CLIENT' AND o.counterparty_id=$2 AND o.status<>'CANCELLED' ORDER BY o.due_date NULLS LAST,o.created_at`, [project.id, user.clientId]),
    query<Record<string, unknown>>("SELECT id,claimed_amount_kopecks,confirmed_amount_kopecks,payment_method,status,claimed_at,received_at,rejection_comment FROM client_payment_claims WHERE project_id=$1 AND client_id=$2 ORDER BY created_at DESC", [project.id, user.clientId]),
    query<Record<string, unknown>>("SELECT id,amount_kopecks,transaction_date,title FROM financial_transactions WHERE project_id=$1 AND client_id=$2 AND type='INCOME' ORDER BY transaction_date DESC", [project.id, user.clientId]),
    first<{ total: number | string }>("SELECT COALESCE(SUM(remaining_kopecks),0) total FROM client_unapplied_funds WHERE project_id=$1 AND client_id=$2", [project.id, user.clientId]),
    query(`SELECT a.id,a.original_filename filename,a.category,a.created_at,'/api/client/files/'||a.id url FROM attachments a WHERE a.project_id=$1 AND a.upload_status='LINKED' AND a.deleted_at IS NULL AND (a.client_visible=1 OR a.visibility='CLIENT') AND a.category IN('ESTIMATE','CONTRACT','CONTRACT_PDF','SIGNED_CONTRACT','FINAL_ALBUM','OTHER') ORDER BY a.created_at DESC`, [project.id]),
    listClientAdditionalWorks(user, project.id), additionalWorkCommercialSummary(project.id), first<Record<string, unknown>>("SELECT id,status,accepted_at FROM project_handovers WHERE project_id=$1 AND status IN('AWAITING_CLIENT_INSPECTION','CORRECTIONS_REQUIRED','REINSPECTION_REQUIRED','ACCEPTED')", [project.id]), first<{ id: string }>("SELECT id FROM apartment_passports WHERE project_id=$1 AND client_id=$2 AND status='PUBLISHED' AND current_published_version_id IS NOT NULL", [project.id, user.clientId]),
  ]);
  const safeStages = stages.map((stage) => ({ id: stage.id, name: stage.name, status: stage.status, progressPercent: Math.round(Number(stage.progress_percent)), acceptanceStatus: stage.acceptance_status, acceptanceRequired: Boolean(stage.client_acceptance_required), tasks: tasks.filter((task) => task.stage_id === stage.id).map((task) => ({ id: task.id, name: task.title, status: task.status, progressPercent: taskProgress({ progress_type: task.progress_type as "BINARY" | "QUANTITY", status: String(task.status), planned_quantity: task.planned_quantity as number | null, completed_quantity: task.completed_quantity as number | null }) })) }));
  const dueKopecks = obligations.reduce((sum, item) => sum + Number(item.remaining_kopecks), 0);
  const completed = String(handover?.status ?? "") === "ACCEPTED";
  const latestPhotos = reports.flatMap((report) => Array.isArray((report as Record<string, unknown>).photos) ? (report as Record<string, unknown>).photos as Record<string, unknown>[] : []).slice(0, 5);
  const currentPhase = completed ? "Ремонт завершён" : String(safeStages.find((stage) => !["COMPLETED", "CANCELLED"].includes(String(stage.status)))?.name ?? (safeStages.length ? "Завершающие работы" : "Планирование работ"));
  const action = selectClientPortalAction({ handover, additionalWorks: additionalWorks as unknown as Record<string, unknown>[], stages, dueKopecks, claims });
  return { user: { name: user.clientName }, objects, current: { ...selected, mode: completed ? "COMPLETED" : "PROJECT", serviceLabel: completed ? "Квартира сдана" : "Ремонт", serviceStatus: completed ? "COMPLETED" : "ACTIVE", passportAvailable: Boolean(passport), progress: completed ? 100 : overall, progressLabel: completed ? "Ремонт завершён" : "Общая готовность квартиры", currentPhase, action, latestUpdate: reports[0] ? { type: "DAILY_REPORT", title: String((reports[0] as Record<string, unknown>).comment ?? "Добавлен новый фотоотчёт"), occurredAt: Number((reports[0] as Record<string, unknown>).report_date) } : null, latestPhotos, designProgress: matchingDesign?.progress ?? null, productionProgress, publishedForecast: project.published_forecast_end_date, stages: safeStages, delays, reports, hiddenWorks, additionalWorks, handover, commercial, payments: { dueKopecks, confirmedKopecks: history.reduce((sum, item) => sum + Number(item.amount_kopecks), 0), contractAmountKopecks: project.contract_amount_kopecks, agreedTotalKopecks: Number(commercial?.currentCommercialWorksKopecks ?? project.contract_amount_kopecks), obligations, claims, history, unappliedKopecks: Number(unapplied?.total ?? 0) }, documents } };
}

export async function getClientPortalPhotos(user: ClientPortalUser, projectId: string, rawOffset = 0) {
  const offset = Math.max(0, Math.floor(Number(rawOffset) || 0)), limit = 24;
  const rows = await query<Record<string, unknown>>(`SELECT a.id,a.original_filename filename,'/api/client/files/'||a.id url,dr.id report_id,dr.report_date,CASE WHEN dr.comment_client_visible=1 THEN dr.comment ELSE NULL END comment
    FROM attachments a JOIN daily_reports dr ON dr.id=a.entity_id AND a.entity_type='DailyReport' JOIN projects p ON p.id=dr.project_id
    WHERE p.id=$1 AND p.client_id=$2 AND a.upload_status='LINKED' AND a.deleted_at IS NULL AND (a.visibility='CLIENT' OR a.client_visible=1)
    ORDER BY dr.report_date DESC,a.created_at DESC,a.id LIMIT $3 OFFSET $4`, [projectId, user.clientId, limit + 1, offset]);
