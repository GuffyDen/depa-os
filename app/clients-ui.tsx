...
type ClientDetail = {
  ...
  orders: { id: string; number: string; title: string; status: string; amountKopecks: number | null }[];
...
type ClientTab = "overview" | "portal" | "crm" | "projects" | "orders" | "estimates" | "contracts" | "finances" | "tasks" | "documents";
...
function ClientPortalAccessPanel({client,enabled}:{client:Client;enabled:boolean}){const [data,setData]=useState<Record<string,unknown>|null>(null),[invite,setInvite]=useState(""),[error,setError]=useState(""),[busy,setBusy]=useState(false);async function load(){try{setData(await json(await fetch(`/api/client-portal/access?clientId=${encodeURIComponent(client.id)}`,{cache:"no-store"})))}catch(reason){setError(reason instanceof Error?reason.message:"Не удалось загрузить доступ.")}}useEffect(()=>{void load()},[client.id]);async function act(action:"CREATE"|"RESET"|"DISABLE"){if(action!=="CREATE"&&!window.confirm(action==="RESET"?"Отозвать все сессии и создать новое приглашение?":"Отключить вход в личный кабинет?"))return;setBusy(true);setError("");try{const result=await json<{inviteUrl?:string}>(await fetch("/api/client-portal/access",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({clientId:client.id,action})}));if(result.inviteUrl)setInvite(`${window.location.origin}${result.inviteUrl}`);await load()}catch(reason){setError(reason instanceof Error?reason.message:"Не удалось изменить доступ.")}finally{setBusy(false)}}if(!enabled)return <div className="client-tab-empty"><h4>Нет права управлять личным кабинетом.</h4></div>;const status=String(data?.status??"NOT_CREATED");return <div className="portal-access panel"><header><div><span className="eyebrow">ЛИЧНЫЙ КАБИНЕТ</span><h3>{status==="ACTIVE"?"Доступ активен":status==="DISABLED"?"Доступ отключён":"Доступ не создан"}</h3></div><span className={`client-status ${status==="ACTIVE"?"active":"archived"}`}>{status}</span></header><div className="portal-access-facts"><span>Логин <b>{String(data?.login_identifier??client.phone)}</b></span><span>Последний вход <b>{data?.last_login_at?formatDate(Number(data.last_login_at)):"—"}</b></span><span>Активные сессии <b>{String(data?.active_sessions??0)}</b></span><span>Приглашение до <b>{data?.invite_expires_at?formatDate(Number(data.invite_expires_at)):"—"}</b></span></div>{invite?<div className="portal-invite"><span>Одноразовая ссылка на 72 часа</span><code>{invite}</code><button className="secondary" onClick={()=>navigator.clipboard?.writeText(invite)}>Скопировать ссылку</button><small>Отправьте клиенту вручную через Telegram, WhatsApp или SMS.</small></div>:null}{error?<div className="form-error">{error}</div>:null}<div className="portal-access-actions">{status!=="ACTIVE"?<button className="primary" disabled={busy} onClick={()=>void act("CREATE")}>Создать доступ</button>:<><button className="secondary" disabled={busy} onClick={()=>void act("RESET")}>Сбросить доступ</button><button className="link danger-text" disabled={busy} onClick={()=>void act("DISABLE")}>Отключить</button></>}</div></div>}
...
function ClientCard({ clientId, access, onClose, onChanged, onEdit, onOpenProject, onOpenLead, onOpenOrder, onCreateOrder, onOpenEstimate, onCreateEstimate,onOpenContract }: { clientId: string; access: AccessProfile; onClose: () => void; onChanged: () => void; onEdit: (client: Client) => void; onOpenProject?: (id: string) => void; onOpenLead?:(id:string)=>void; onOpenOrder?:(id:string)=>void; onCreateOrder?:(clientId:string)=>void; onOpenEstimate?:(id:string)=>void; onCreateEstimate?:(context:{clientId:string;responsibleUserId:string})=>void;onOpenContract?:(id:string)=>void }) {
  ...
  useEffect(()=>{if(!access.actions["estimates.view"])return;const controller=new AbortController();fetch(`/api/estimates?clientId=${encodeURIComponent(clientId)}&limit=100`,{cache:"no-store",signal:controller.signal}).then(response=>json<{items:typeof estimates}>(response)).then(value=>setEstimates(value.items)).catch(()=>undefined);return()=>controller.abort()},[clientId,access]);
...
export function ClientsScreen({ currentUser, access, initialClientId = null, onClientClosed, onOpenProject,onOpenLead,onOpenOrder,onCreateOrder,onOpenEstimate,onCreateEstimate,onOpenContract }: { currentUser: AuthUser; access: AccessProfile; initialClientId?: string | null; onClientClosed?: () => void; onOpenProject?: (id: string) => void;onOpenLead?:(id:string)=>void;onOpenOrder?:(id:string)=>void;onCreateOrder?:(clientId:string)=>void;onOpenEstimate?:(id:string)=>void;onCreateEstimate?:(context:{clientId:string;responsibleUserId:string})=>void;onOpenContract?:(id:string)=>void }) {
  ...
  return <section className="screen-section clients-screen">
    ...
    <div className="panel client-list-panel">
      ...
      {items.length ? <div className="client-table"><div className="client-row client-head"><span>Клиент</span><span>Телефон</span><span>Статус</span><span>Ответственный</span><span className="client-col-source">Источник</span><span className="client-col-contact">Связь</span><span>Объекты</span><span className="client-col-updated">Обновлён</span><span className="client-col-created">Создан</span><span aria-hidden="true" /></div>{items.map((client) => <button className="client-row" key={client.id} onClick={() => setOpenClientId(client.id)}><span className="client-name"><i>{initials(client.fullName)}</i><span><b>{client.fullName}</b><small>{client.email || "Email не указан"}</small></span></span><span>{client.phone}</span><span><em className={`client-status ${client.status.toLowerCase()}`}>{client.status === "ACTIVE" ? "Активный" : "Архивный"}</em></span><span>{client.responsibleName}</span><span className="client-col-source">{SOURCE_LABELS[client.source] || client.source}</span><span className="client-col-contact">{client.preferredContact === "EMAIL" ? "Email" : client.preferredContact === "PHONE" ? "Телефон" : "Не указано"}</span><span>{client.projectCount}</span><span className="client-col-updated">{formatDate(client.updatedAt)}</span><span className="client-col-created">{formatDate(client.createdAt)}</span><span className="client-row-action" aria-hidden="true">›</span></button>)}</div> : null}
