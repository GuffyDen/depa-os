...
function ClientPaymentInbox({cashboxes,onChanged}:{cashboxes:FinanceData["cashboxes"];onChanged:()=>void}){const [claims,setClaims]=useState<{id:string;client_name:string;project_name:string;claimed_amount_kopecks:number;confirmed_amount_kopecks:number|null;payment_method:string|null;client_comment:string|null;status:string;claimed_at:number;proof_attachment_id:string|null}[]>([]),[allowed,setAllowed]=useState(true),[error,setError]=useState("");async function load(){const response=await fetch("/api/client-payments",{cache:"no-store"});if(response.status===403){setAllowed(false);return}const result=await response.json();if(!response.ok)throw new Error(result.error);setClaims(result.items??[])}useEffect(()=>{void load().catch(reason=>setError(reason instanceof Error?reason.message:"Не удалось загрузить заявления."))},[]);async function confirm(claim:typeof claims[number]){const amount=window.prompt("Фактически получено, ₽",String(claim.claimed_amount_kopecks/100));if(!amount)return;const options=cashboxes.filter(box=>box.status==="ACTIVE");const cashboxId=window.prompt(`ID кассы:\n${options.map(box=>`${box.id} — ${box.name}`).join("\n")}`,options[0]?.id??"");if(!cashboxId)return;const receivedAt=Math.floor(new Date(`${new Date().toISOString().slice(0,10)}T12:00:00+10:00`).getTime()/1000);const response=await fetch("/api/client-payments",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"CONFIRM",claimId:claim.id,actualAmountKopecks:Math.round(Number(amount.replace(",","."))*100),cashboxId,receivedAt})}),result=await response.json();if(!response.ok){setError(result.error);return}await load();onChanged()}async function reject(claim:typeof claims[number]){const comment=window.prompt("Причина, которую увидит клиент");if(!comment)return;const response=await fetch("/api/client-payments",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"REJECT",claimId:claim.id,comment})}),result=await response.json();if(!response.ok){setError(result.error);return}await load()}if(!allowed)return null;const pending=claims.filter(claim=>claim.status==="PENDING");return <section className="panel client-payment-inbox"><div className="table-toolbar"><strong>Оплаты клиентов</strong><small>Ожидают подтверждения: {pending.length}</small></div>{error?<div className="form-error">{error}</div>:null}{pending.length?pending.map(claim=><article key={claim.id}><div><b>{claim.client_name}</b><span>{claim.project_name} · {new Date(claim.claimed_at*1000).toLocaleString("ru-RU")}</span><small>{claim.client_comment||"Без комментария"}</small></div><strong>{money(claim.claimed_amount_kopecks)}</strong><div>{claim.proof_attachment_id?<a className="link" href={`/api/files/${claim.proof_attachment_id}`} target="_blank" rel="noreferrer">Открыть proof</a>:<span>Без файла</span>}<button className="secondary" onClick={()=>void reject(claim)}>Не подтверждено</button><button className="primary" onClick={()=>void confirm(claim)}>Подтвердить получение</button></div></article>):<div className="finance-empty">Ожидающих подтверждения оплат нет.</div>}</section>}
...
export function FinanceScreen({ onNew }: { onNew: (mode: FinanceMode) => void }) {
  ...
  return <section className="screen-section finance-screen"><div className="screen-intro"><div><span className="eyebrow">ЕДИНЫЙ УЧЁТ</span><h2>Финансы</h2><p>Кассы показывают физическое расположение денег. Просмотр касс не даёт права проводить операции из чужой кассы.</p></div><div className="finance-actions">{data?.capabilities.createIncome && canWrite ? <button className="secondary" onClick={() => onNew("INCOME")}>＋ Поступление</button> : null}{data?.capabilities.createExpense && canWrite ? <button className="secondary" onClick={() => onNew("EXPENSE")}>− Расход</button> : null}{data?.capabilities.createTransfer && canWrite && data.transferRecipients.length ? <button className="primary" onClick={() => onNew("TRANSFER")}>⇄ Переместить</button> : null}</div></div>
    ...
    {data && <>
      <ClientPaymentInbox cashboxes={data.cashboxes} onChanged={refresh}/>
