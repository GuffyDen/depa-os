...
import { createFinanceAttachmentDraft, financePromiseWithTimeout, FINANCE_ATTACHMENT_ACCEPT, logFinanceLifecycle, uploadFinanceAttachment, type FinanceAttachmentDraft } from "../lib/finance-attachments-client";
...
type FinanceAttachmentItem = { id: string; originalFilename: string; mimeType: string; sizeBytes: number; status: "PENDING" | "LINKED" | "FAILED"; createdAt: number; failureCode: string | null; attemptCount: number; lastFailureAt: number | null };
...
type FinanceTransaction = {
  ...
  attachments: FinanceAttachmentItem[];
...
function visibleFinanceAttachments(items: FinanceAttachmentItem[]) {
  return items.filter((item, index) => {
    if (item.status !== "FAILED") return true;
    const normalizedName = item.originalFilename.trim().toLocaleLowerCase("en-US");
    return !items.some((candidate, candidateIndex) => candidateIndex !== index
      && candidate.originalFilename.trim().toLocaleLowerCase("en-US") === normalizedName
      && (candidate.createdAt > item.createdAt || candidate.createdAt === item.createdAt && candidate.id > item.id));
  });
}

function failedAttachmentMessage(attachment: FinanceAttachmentItem) {
  if (attachment.failureCode?.startsWith("HEIC_")) return "Формат HEIC не удалось обработать. Операция сохранена без вложения.";
  return "Не удалось обработать фото. Операция сохранена без вложения.";
}
...
function TransactionDetailModal({ transaction, canEdit, canAcceptAttention, onClose, onSaved, onAttachmentsChanged, onAttentionChanged }: { transaction: FinanceTransaction; canEdit: boolean; canAcceptAttention: boolean; onClose: () => void; onSaved: () => void; onAttachmentsChanged: () => void | Promise<void>; onAttentionChanged: () => void | Promise<void> }) {
  ...
  const receiptIssue = missingReceiptIssue(transaction);
  const displayedAttachments = useMemo(() => visibleFinanceAttachments(attachments), [attachments]);
  ...
  async function addAttachments(files: File[], retryAttachment?: FinanceAttachmentItem) {
    if (!files.length || uploading) return;
    setError(""); setUploadNotice("");
    let pairs: { file: File; draft: FinanceAttachmentDraft }[];
    try {
      const selectedFiles = retryAttachment ? files.slice(0, 1) : files;
      pairs = await Promise.all(selectedFiles.map(async (file) => ({ file, draft: await createFinanceAttachmentDraft(file, retryAttachment?.id) })));
    }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Файл не прошёл проверку."); return; }
    setUploading(true);
    try {
      const slotResponse = await fetch("/api/finance/attachments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ transactionId: transaction.id, retryAttachmentId: retryAttachment?.id, attachments: pairs.map((item) => item.draft) }) });
      ...
      setAttachments((current) => retryAttachment
        ? current.map((item) => item.id === retryAttachment.id ? { ...item, originalFilename: pairs[0].draft.originalFilename, mimeType: pairs[0].draft.mimeType, sizeBytes: 0, status: "PENDING", failureCode: null, attemptCount: item.attemptCount + 1 } : item)
        : [...current, ...pairs.map(({ draft }) => ({ id: draft.attachmentId, originalFilename: draft.originalFilename, mimeType: draft.mimeType, sizeBytes: 0, status: "PENDING" as const, createdAt: Math.floor(Date.now() / 1000), failureCode: null, attemptCount: 1, lastFailureAt: null }))]);
      ...
      setAttachments((current) => current.map((attachment) => {
        const pairIndex = pairs.findIndex(({ draft }) => draft.attachmentId === attachment.id);
        const result = pairIndex >= 0 ? results[pairIndex] : null;
        if (!result || result.status !== "rejected") return attachment;
        return { ...attachment, failureCode: result.reason && typeof result.reason === "object" && "code" in result.reason ? String(result.reason.code) : "PROCESS_TIMEOUT", lastFailureAt: Math.floor(Date.now() / 1000) };
      }));
  ...
  return <div className="modal-wrap" onMouseDown={(event) => { if (event.target === event.currentTarget && !uploading && !attentionLoading) onClose(); }}><section className="modal finance-detail-modal" role="dialog" aria-modal="true" aria-labelledby="transaction-detail-title" aria-hidden={confirmAcceptance || undefined} inert={confirmAcceptance}><div className="modal-head"><div><span className="eyebrow">ФИНАНСОВАЯ ОПЕРАЦИЯ</span><h3 id="transaction-detail-title">{transactionLabel(transaction)}</h3></div><button onClick={onClose} aria-label="Закрыть">×</button></div><div className="transaction-facts"><span>Описание <b>{transaction.title}</b></span><span>Сумма <b>{money(transaction.amountKopecks)}</b></span><span>Дата <b>{new Date(transaction.transactionDate * 1000).toLocaleDateString("ru-RU")}</b></span><span>Категория <b>{financeCategoryLabel(transaction.category)}</b></span><span>Источник <b>{transactionSourceLabel(transaction)}</b></span><span>Автор <b>{transaction.authorName}</b></span></div>{transaction.allocations.length > 0 && <div className="detail-allocations">{transaction.allocations.map((item) => <span key={item.id}>{item.projectName}<b>{money(item.amountKopecks)}</b></span>)}</div>}
    ...
    <section className="finance-attachments"><header><div><span className="eyebrow">ДОКУМЕНТЫ / ВЛОЖЕНИЯ</span><b>{attachments.filter((item) => item.status === "LINKED").length} прикреплено</b></div><label className="secondary finance-attachment-add"><input type="file" multiple accept={FINANCE_ATTACHMENT_ACCEPT} disabled={uploading} onChange={(event) => { void addAttachments(Array.from(event.target.files ?? [])); event.currentTarget.value = ""; }} />{uploading ? "Загружаем…" : "＋ Добавить чек / фото"}</label></header>
      <div className="finance-attachment-list">{displayedAttachments.length ? displayedAttachments.map((attachment) => <article key={attachment.id}><i>{attachment.mimeType === "application/pdf" ? "PDF" : "IMG"}</i><span><b>{attachment.originalFilename}</b><small>{attachment.status === "PENDING" ? `Чек загружается… · попытка ${attachment.attemptCount}` : attachment.status === "FAILED" ? failedAttachmentMessage(attachment) : `Чек прикреплён · ${fileSize(attachment.sizeBytes)}`}</small></span>{attachment.status === "LINKED" ? <a href={`/api/files/${attachment.id}`} target="_blank" rel="noreferrer">Открыть</a> : attachment.status === "FAILED" ? <label className="link finance-attachment-retry"><input type="file" accept={FINANCE_ATTACHMENT_ACCEPT} disabled={uploading} onChange={(event) => { void addAttachments(Array.from(event.target.files ?? []), attachment); event.currentTarget.value = ""; }} />Повторить</label> : <em>Загрузка…</em>}</article>) : <p>Файлов пока нет. Операция сохранена без чека.</p>}</div>
...
export function FinanceOperationModal({ mode, onClose, onSaved, initialProjectId = "", initialClientId = "",initialOrderId="",initialOrderNumber="",initialAmount="",initialTitle="" }: { mode: FinanceMode; onClose: () => void; onSaved: () => void; initialProjectId?: string; initialClientId?: string;initialOrderId?:string;initialOrderNumber?:string;initialAmount?:string;initialTitle?:string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    logFinanceLifecycle("submit_clicked", { traceId, mode, attachmentCount: selectedAttachments.length });
    if (!formElement.checkValidity()) {
      logFinanceLifecycle("form_validation_failed", { traceId, mode });
      formElement.reportValidity();
      return;
    }
    if (submittingRef.current) return;
    submittingRef.current = true; setError(""); setLoading(true);
    try {
      const form = new FormData(formElement);
      logFinanceLifecycle("form_validation_done", { traceId, mode, durationMs: Math.round(performance.now() - submitStartedAt) });
      ...
      logFinanceLifecycle("transaction_create_request_started", { traceId, mode, attachmentCount: selectedAttachments.length });
      const response = await financePromiseWithTimeout(fetch("/api/finance", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
      ...
      }) }), FINANCE_TRANSACTION_TIMEOUT_MS, "Сервер слишком долго создаёт операцию. Повторите отправку — дубликат не появится.");
      logFinanceLifecycle("transaction_create_response_received", { traceId, mode, status: response.status, durationMs: Math.round(performance.now() - transactionStartedAt) });
      ...
      const transactionId = result.operation.id;
      const attachments = [...selectedAttachments];
      logFinanceLifecycle("transaction_id_obtained", { traceId, transactionId, durationMs: Math.round(performance.now() - submitStartedAt), idempotent: Boolean(result.operation.idempotent) });
      ...
      onSaved();
      logFinanceLifecycle("ui_success_state", { traceId, transactionId, attachmentCount: attachments.length, hasWarning: Boolean(result.operation.warning) });
      onClose();
      logFinanceLifecycle("modal_closed", { traceId, transactionId, durationMs: Math.round(performance.now() - submitStartedAt) });
      if (!attachments.length) return;
      setTimeout(() => {
        logFinanceLifecycle("attachment_background_started", { traceId, transactionId, attachmentCount: attachments.length });
        void Promise.allSettled(attachments.map(({ file, draft }) => uploadFinanceAttachment({ file, draft, transactionId, projectId: receiptProjectId, traceId }))).then((results) => {
          const failed = results.filter((item) => item.status === "rejected").length;
          logFinanceLifecycle("attachment_background_finished", { traceId, transactionId, attachmentCount: attachments.length, failedCount: failed });
          onSaved();
        });
      }, 0);
    } catch (reason) {
      logFinanceLifecycle("transaction_create_failed", { traceId, mode, durationMs: Math.round(performance.now() - submitStartedAt), error: reason instanceof Error ? reason.message : "UNKNOWN_ERROR" });
      setError(reason instanceof Error ? reason.message : "Не удалось провести операцию."); setLoading(false); submittingRef.current = false;
    }
  }
  ...
  return <div className="modal-wrap" onMouseDown={(event) => { if (event.target === event.currentTarget && !loading) onClose(); }}><section className="modal finance-modal" role="dialog" aria-modal="true" aria-labelledby="finance-modal-title"><div className="modal-head"><div><span className="eyebrow">ФИНАНСЫ</span><h3 id="finance-modal-title">{modeLabels[mode]}</h3></div><button onClick={onClose} aria-label="Закрыть">×</button></div>
    <form onSubmit={submit}>
      ...
      <label className="upload"><input name="attachment" type="file" multiple accept={FINANCE_ATTACHMENT_ACCEPT} onChange={(event) => { const files = Array.from(event.target.files ?? []); void Promise.all(files.map(async (file) => ({ file, draft: await createFinanceAttachmentDraft(file) }))).then((next) => { setSelectedAttachments(next); setError(""); }).catch((reason) => { setSelectedAttachments([]); setError(reason instanceof Error ? reason.message : "Файл не прошёл проверку."); }); }} /><i>＋</i><span><b>{selectedAttachments.length ? `${selectedAttachments.length} ${selectedAttachments.length === 1 ? "файл выбран" : "файла выбрано"}` : "Прикрепить чек / фото"}</b><small>До 10 файлов · изображения до 25 МБ, PDF до 10 МБ · необязательно</small></span></label>
